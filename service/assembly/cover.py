...
def build_digital_cover_page(child_name, gender, asset_root, target_w_mm, target_h_mm, out_path, dpi=150):
    ...
    # do cleanly). Native panel aspect (~0.93) is close to but not
    # identical to the digital page aspect (~0.986) -- a small stretch,
    # same tradeoff already accepted for the print cover's spine.
    render_scale = 2
    target_w_px = round(target_w_mm / 25.4 * dpi)
    target_h_px = round(target_h_mm / 25.4 * dpi)
    pix = native_page.get_pixmap(matrix=fitz.Matrix(dpi / 72 * render_scale, dpi / 72 * render_scale))
    native_doc.close()
