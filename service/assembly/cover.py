...
def render_title_png(line1, line2, gender, font_path, out_path):
    # PIL's stroke rendering doesn't anti-alias consistently at native
    # resolution (verified: alpha jumps straight 0->255 with no smooth
    # transition at most edges), which shows up as a faint blocky/faceted
    # halo around the stroke. Rendering at a higher resolution and
    # downsampling with a high-quality filter (LANCZOS) forces smooth
    # edges regardless of the rasterizer's own antialiasing quality --
    # standard supersampling technique.
    SUPERSAMPLE = 4

    font_size_px = round(FONT_SIZE_PT / 72 * DPI) * SUPERSAMPLE
    font = ImageFont.truetype(font_path, font_size_px)
    leading_px = round(LEADING_PT / 72 * DPI) * SUPERSAMPLE
    stroke_px = STROKE_PX_AT_300DPI * SUPERSAMPLE
    ...
    canvas_w = int(max(w1, w2) + stroke_px * 4 + 40 * SUPERSAMPLE)
    canvas_h = int(leading_px * 2 + stroke_px * 4 + 40 * SUPERSAMPLE)
    img = Image.new("RGBA", (canvas_w, canvas_h), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    cx = canvas_w / 2
    draw_tracked_line(draw, line1, cx, 20 * SUPERSAMPLE)
    draw_tracked_line(draw, line2, cx, 20 * SUPERSAMPLE + leading_px)

    bbox = img.getbbox()
    img = img.crop(bbox)
    # downsample back to the true target resolution -- this is the step
    # that actually smooths out the hard/blocky edges
