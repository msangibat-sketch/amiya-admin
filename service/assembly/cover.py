...
def build_digital_cover_page(child_name, gender, asset_root, target_w_mm, target_h_mm, out_path, dpi=150):
    """
    Builds a single flat cover page for the DIGITAL reader (Heyzine) --
    just the front panel of the print cover, with no spine/back
    panel/groove needed (a flipbook's first page just IS the cover).
    Resized to match the digital reader's other page dimensions exactly,
    so it flips cleanly alongside them rather than looking like an oddly
    sized inserted page.
    """
    import io
    ...
    # crop just the front panel from the source template, at native size
    src_doc = fitz.open(blank_cover_path)
    src_page = src_doc[0]
    pix = src_page.get_pixmap(matrix=fitz.Matrix(DPI / 72, DPI / 72))
    img = Image.frombytes("RGB", (pix.width, pix.height), pix.samples)
    src_doc.close()
    ...
    panel_w_px = round(PANEL_W_MM * mm_to_px)
    front_panel = img.crop((spine_x1_px, 0, img.width, img.height)).resize((panel_w_px, img.height), Image.LANCZOS)
    front_panel_png = "/tmp/_digital_cover_panel.png"
    front_panel.save(front_panel_png)

    # build at native panel size (240x258mm) with vector title + logo --
    # same positioning as the print cover's front panel, just starting at
    # x=0 since there's no back panel/spine to its left here
    front_safe_cx_mm = GROOVE_MM + SAFE_W_MM / 2

    native_doc = fitz.open()
    native_page = native_doc.new_page(width=PANEL_W_MM * MM_TO_PT, height=TOTAL_H_MM * MM_TO_PT)
    native_page.insert_image(native_page.rect, filename=front_panel_png)
    draw_title(native_page, "МИНИЙ НЭР", child_name.upper(), gender, font_path,
               cx_mm=front_safe_cx_mm, top_mm=TITLE_TOP_MM)
    lx0_mm = front_safe_cx_mm - LOGO_W_MM / 2
    ly0_mm = LOGO_TOP_MM
    lrect = fitz.Rect(lx0_mm * MM_TO_PT, ly0_mm * MM_TO_PT,
                       (lx0_mm + LOGO_W_MM) * MM_TO_PT, (ly0_mm + LOGO_H_MM) * MM_TO_PT)
    native_page.insert_image(lrect, filename=logo_path)

    # Rasterize at 2x the target resolution then downscale -- same
    # supersampling approach used to keep the vector title crisp when it
    # necessarily ends up embedded as a raster JPEG here (unlike the
    # print cover, this page needs a non-uniform resize to match the
    # digital reader's page dimensions, which vector content alone can't
    # do cleanly).
    #
    # Clipped to just the safe zone (207x218mm), not the full 240x258mm
    # panel -- the panel's extra 20mm margin exists only so the print
    # cover can physically wrap around the hardcover board, and the
    # groove is only meaningful next to a spine. Neither means anything
    # on a screen; including them just showed as excess empty space
    # around the title/logo/illustration instead of a cover that fills
    # the frame the way a real book cover does.
    safe_zone_x0_mm = GROOVE_MM
    safe_zone_y0_mm = MARGIN_MM
    clip_rect = fitz.Rect(
        safe_zone_x0_mm * MM_TO_PT, safe_zone_y0_mm * MM_TO_PT,
        (safe_zone_x0_mm + SAFE_W_MM) * MM_TO_PT, (safe_zone_y0_mm + SAFE_H_MM) * MM_TO_PT,
    )

    render_scale = 2
    target_w_px = round(target_w_mm / 25.4 * dpi)
    target_h_px = round(target_h_mm / 25.4 * dpi)
    pix = native_page.get_pixmap(clip=clip_rect, matrix=fitz.Matrix(dpi / 72 * render_scale, dpi / 72 * render_scale))
    native_doc.close()
    raw_img = Image.frombytes("RGB", (pix.width, pix.height), pix.samples)
    final_img = raw_img.resize((target_w_px, target_h_px), Image.LANCZOS)
    ...
    out_doc = fitz.open()
    out_page = out_doc.new_page(width=target_w_mm * MM_TO_PT, height=target_h_mm * MM_TO_PT)
