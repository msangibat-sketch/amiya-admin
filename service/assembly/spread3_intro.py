import fitz
from .config import FONT_PLAYPEN

MM_TO_PT = 72 / 25.4
BLEED_MM = 3.0
PAGE_TRIM_MM = 210.0
MARGIN_MM = 20.0

FONT_PATH = FONT_PLAYPEN
FONT_SIZE = 18
LINEHEIGHT = 30 / 18

LEFT_MARGIN_X0 = BLEED_MM + MARGIN_MM
LEFT_MARGIN_X1 = BLEED_MM + PAGE_TRIM_MM - MARGIN_MM
LEFT_MARGIN_Y0 = BLEED_MM + MARGIN_MM
LEFT_MARGIN_Y1 = BLEED_MM + PAGE_TRIM_MM - MARGIN_MM

# base text uses "охин"/"хүү" (girl/boy) and the correct closing animal
# for each gender -- same lion/polar-bear pairing already used in farewell.py
ANIMAL_BY_GENDER = {"girl": "Цагаан баавгай", "boy": "Арслан"}

SPREAD3_TEXT_TEMPLATE = (
    "Өндөр модод нууцаа шивнэлдэж, \n"
    "\tөнгө өнгийн цэцэгс сэвшээ салхинд бүжиглэдэг,\n"
    "ид шидийн ойн гүнд элдэв янзын амьтад \n"
    "\tамар жимэр амьдардаг байлаа.\n"
    "Нэгэн өдөр ид шидийн ойн өнцөг булан бүрт \n"
    "\tгайхалтай мэдээ түгэв... \n"
    "\u201cОнцгой нэртэй, өхөөрдөм {gender} мэндэлжээ, \n"
    "\tтүүний нэр юунаас бүтсэнийг хамтдаа мэдэцгээе!\u201d \n"
    "хэмээн {animal} зарлав."
)


def build_spread3(spread_path, gender, out_path):
    gender_word = "охин" if gender == "girl" else "хүү"
    animal = ANIMAL_BY_GENDER[gender]
    text = SPREAD3_TEXT_TEMPLATE.format(gender=gender_word, animal=animal)
    lines = text.split("\n")

    doc = fitz.open(spread_path)
    page = doc[0]

    font = fitz.Font(fontfile=FONT_PATH)
    indent_pt = FONT_SIZE * 1.8
    leading_pt = FONT_SIZE * LINEHEIGHT

    # measure natural block width/height (preserving each line's own indent)
    line_widths = []
    for line in lines:
        is_indented = line.startswith("\t")
        clean_line = line.lstrip("\t")
        w = font.text_length(clean_line, fontsize=FONT_SIZE) + (indent_pt if is_indented else 0)
        line_widths.append(w)
    block_w_pt = max(line_widths)
    block_h_pt = leading_pt * len(lines)

    margin_w_pt = (LEFT_MARGIN_X1 - LEFT_MARGIN_X0) * MM_TO_PT
    margin_h_pt = (LEFT_MARGIN_Y1 - LEFT_MARGIN_Y0) * MM_TO_PT

    block_x0 = LEFT_MARGIN_X0 * MM_TO_PT + max(0, (margin_w_pt - block_w_pt) / 2)
    block_y0 = LEFT_MARGIN_Y0 * MM_TO_PT + max(0, (margin_h_pt - block_h_pt) / 2)

    y_cursor = block_y0 + FONT_SIZE  # first baseline
    for line in lines:
        is_indented = line.startswith("\t")
        clean_line = line.lstrip("\t")
        x = block_x0 + (indent_pt if is_indented else 0)
        page.insert_text(fitz.Point(x, y_cursor), clean_line, fontsize=FONT_SIZE,
                          fontfile=FONT_PATH, fontname="playpen", color=(0.15, 0.1, 0.05))
        y_cursor += leading_pt

    doc.save(out_path)
    doc.close()
    print(f"Saved {out_path}")


if __name__ == "__main__":
    build_spread3("/mnt/user-data/uploads/Spread_3_-_Intro_girl_.pdf", "girl",
                  "/home/claude/amiyaa_test/spread3_test.pdf")
