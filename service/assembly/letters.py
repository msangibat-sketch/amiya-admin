...
def size_mult_for(case, kind):
    if kind == "dash":
        return DASH_SIZE_MULT
...
def compute_fixed_layout(analyzed):
    ...
    items_for_sizing = [(size_mult_for(case, kind), aspect) for (_, case, _, aspect, _, kind) in analyzed]
    ...
    layout = []
    x_cursor = ZONE_X0_MM
    letter_visual_h_mm = H * TYPICAL_LETTER_VISUAL_RATIO
    for (key, case, cropped, aspect, variant, kind) in analyzed:
        if kind == "dash":
            ...
            h_mm = H * size_mult_for(case, kind)
            w_mm = h_mm * aspect
...
def draw_accumulation(page, given_items, full_layout, reference_H, is_night, night_letters_dir):
    import os
    baseline_y_mm = ZONE_Y0_MM + BASELINE_FRACTION * ZONE_H_MM
    # center on the VISIBLE letter glyph area, not the full string+letter
    # image height (which is mostly invisible string) -- otherwise the dash
    # centers itself up near the strings instead of between the letters.
    letter_visual_h_mm = reference_H * TYPICAL_LETTER_VISUAL_RATIO
...
def build_letter_sequence(asset_root, letter_variants, gender, out_dir, caption_xlsx_path=None):
    ...
    full_layout, reference_H = compute_fixed_layout(analyzed_all)
    ...
    for key, case, variant, _, kind in name_sequence:
        ...
        draw_accumulation(give_page, analyzed_all[:given_count], full_layout[:given_count],
                           reference_H, is_night, night_letters_dir)
